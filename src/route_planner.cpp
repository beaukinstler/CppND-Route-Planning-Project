...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    // for all the neighboard, link this as the parent, give one more cost as g + 1, and calculate the h value
    // by passing in the neighbor to the calculation function.
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while(current_node != nullptr){
        ...
        current_node = current_node->parent; // move to the parrent. If null, this assumed to be the end.
    }
...
void RoutePlanner::AStarSearch() {
    ...
    int counter = 0;
    while(this->open_list.size() > 0 && counter < 100){
        this->AddNeighbors(current_node);
        current_node = this->NextNode();
        if(current_node->y == this->end_node->x && current_node->y == this->end_node->y){
            break;
        }
        counter += 1;
    }
